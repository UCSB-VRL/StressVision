    ...
    def face_tube(self, idx, video_mat):
        ...
        try:
            if l < w / 2 and x_sum[x_edge[0][-1]] < 50:
                pad[0 : org_img[h - pad_h : h, 0:r].shape[0], w - r : w] = org_img[
                    h - pad_h : h, 0:r
                ]
            elif r > w / 2 and x_sum[x_edge[0][0]] < 50:
                pad[0 : org_img[h - pad_h : h, 0:r].shape[0], 0 : w - l] = org_img[
                    h - pad_h : h, l:r
                ]
            else:
                pad[0 : org_img[h - pad_h : h, 0:r].shape[0], l:r] = org_img[
                    h - pad_h : h, l:r
                ]
        except:
            pad[0 : org_img[h - pad_h : h, 0:r].shape[0], l:r] = org_img[
...
if __name__ == "__main__":
    ...
    for v_file_path in file_list:
        ...
        out2 = cv2.VideoWriter(
            fname2, cv2.VideoWriter_fourcc(*"DIVX"), 15, (w, h), isColor=False
        )
